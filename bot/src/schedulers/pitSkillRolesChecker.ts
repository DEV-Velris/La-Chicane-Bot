  ...
  async () => {
    ...
    usersToCheck.forEach(async (user) => {
      ...
      if (pitSkillData.status === 0) {
        ...
        console.error(`PitSkill API returned an error for user ${user.discordId}`, {
          pitSkillData,
        });
        return;
      }
      ...
      if (userRoles.some((roleId) => rolesToAdd[0].includes(roleId))) {
        ...
        if (licenseRolesId.some((roleId) => roleId === undefined || roleId === null)) {
          console.error(
        ...
        const rolesToRemove = userRoles.filter((roleId) => licenseRolesId.includes(roleId));
        ...
        try {
          ...
          console.error(`Failed to update License role for user ${user.discordId}`, error);
        }
      }
      ...
      if (userRoles.some((roleId) => rolesToAdd[1].includes(roleId))) {
        ...
        if (levelRolesId.some((roleId) => roleId === undefined || roleId === null)) {
          console.error('One or more Level role IDs are not set in environment variables.');
          return;
        }

        // Remove the old Level roles
        const rolesToRemove = userRoles.filter((roleId) => levelRolesId.includes(roleId));
        ...
        try {
          ...
          console.error(`Failed to update Level role for user ${user.discordId}`, error);
        }
      }

      // Check for name changes
      const flagEmoji = flag(pitSkillData.payload.sigma_user_data.profile_data.driverCountry);
      const rename = `[${flagEmoji}] ${pitSkillData.payload.sigma_user_data.profile_data.first_name} ${pitSkillData.payload.sigma_user_data.profile_data.last_name} (${pitSkillData.payload.sigma_user_data.profile_data.shortname})`;

      if (discordUser.nickname !== rename) {
        try {
          if (discordUser.manageable) {
            await discordUser.setNickname(rename, 'PitSkill link rename');
          }
        } catch (error) {
          console.error(`Failed to rename user ${user.discordId} to ${rename}`, error);
        }
      }
    });
  },
